// Reemplaza esta URL por la IP local de la máquina donde corre n8n.
// OJO: si pruebas en un celular físico con Expo Go, "localhost" NO funciona,
// porque el celular busca su propio localhost, no el de tu computadora.
// Usa la IP de red local de tu PC (ej. 192.168.1.34) y asegúrate de que
// el celular y la PC estén conectados al mismo Wi-Fi.
//
// Cómo obtener tu IP local:
//   Windows (PowerShell): ipconfig  -> busca "Dirección IPv4"
//   Mac/Linux: ifconfig o `ipconfig getifaddr en0`
export const N8N_WEBHOOK_URL = 'http://192.168.1.XX:5678/webhook/taxi-request';
